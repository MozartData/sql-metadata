...
def test_preprocess_query():
    ...
    assert preprocess_query("SELECT foo FROM `db`.`test`") == "SELECT foo FROM db.test"
...
def test_get_query_tables():
    ...
    assert ["revision", "page", "wikicities_user"] == get_query_tables(
    ...
    assert ["events"] == get_query_tables(
    ...
    assert ["report_wiki_recent_pageviews", "dimension_wikis"] == get_query_tables(
    ...
    assert ["dimension_wikis", "fact_wam_scores"] == get_query_tables(
        "SELECT DISTINCT dw.lang FROM `dimension_wikis` `dw` INNER JOIN `fact_wam_scores` `fwN` ON ((dw.wiki_id = fwN.wiki_id)) WHERE fwN.time_id = FROM_UNIXTIME(N) ORDER BY dw.lang ASC"
    )

    assert ["fact_wam_scores", "dimension_wikis"] == get_query_tables(
        "SELECT count(fwN.wiki_id) as wam_results_total FROM `fact_wam_scores` `fwN` left join `fact_wam_scores` `fwN` ON ((fwN.wiki_id = fwN.wiki_id) AND (fwN.time_id = FROM_UNIXTIME(N))) left join `dimension_wikis` `dw` ON ((fwN.wiki_id = dw.wiki_id)) WHERE (fwN.time_id = FROM_UNIXTIME(N)) AND (dw.url like X OR dw.title like X) AND fwN.vertical_id IN (XYZ) AND dw.lang = X AND (fwN.wiki_id NOT IN (XYZ)) AND ((dw.url IS NOT NULL AND dw.title IS NOT NULL))"
    )

    assert ["revision", "page", "wikicities_cN.user"] == get_query_tables(
    ...
    assert ["foo_pageviews"] == get_query_tables(
    ...
    assert ["rollup_wiki_pageviews"] == get_query_tables(
    ...
    # INSERT queries
    assert ["0070_insert_ignore_table"] == get_query_tables(
    ...
    assert ["foo"] == get_query_tables("INSERT INTO `foo` (id,text) VALUES (X,X)")

    assert ["page_vote"] == get_query_tables(
        "INSERT /* VoteHelper::addVote xxx */  INTO `page_vote` (article_id,user_id,time) VALUES ('442001','27574631','20180228130846')"
    )

    # UPDATE queries
    assert ["page"] == get_query_tables(
        "UPDATE `page` SET page_touched = X WHERE page_id = X"
    )

    # ORDER BY
    assert ["bar"] == get_query_tables("SELECT foo FROM bar ORDER BY id")

    assert ["bar"] == get_query_tables("SELECT foo FROM bar WHERE id > 20 ORDER BY id")

    assert ["bar"] == get_query_tables("SELECT foo FROM bar ORDER BY id DESC")

    assert ["bar"] == get_query_tables("SELECT foo FROM bar ORDER BY id LIMIT 20")

    # REPLACE queries
    assert ["page_props"] == get_query_tables(
        "REPLACE INTO `page_props` (pp_page,pp_propname,pp_value) VALUES ('47','infoboxes','')"
    )

    # JOINs
    assert ["product_a.users", "product_b.users"] == get_query_tables(
        "SELECT a.* FROM product_a.users AS a JOIN product_b.users AS b ON a.ip_address = b.ip_address"
    )

    # database.schema.table formats
    assert ["MYDB1.MYSCHEMA1.MYTABLE1"] == get_query_tables(
        "SELECT * FROM MYDB1.MYSCHEMA1.MYTABLE1"
    )

    assert ["MYDB1.MYSCHEMA1.MYTABLE1", "MYDB2.MYSCHEMA2.MYTABLE2"] == get_query_tables(
    ...
    assert ["MYDB.MYSCHEMA.MYTABLE"] == get_query_tables(
    ...
    assert ["MYDB1.TABLE1", "MYDB2.TABLE2"] == get_query_tables(
...
def test_get_query_limit_and_offset():
    ...
    assert get_query_limit_and_offset("SELECT foo_limit FROM bar_offset LIMIT 50") == (
        50,
        0,
    )
...
def test_redshift():
    assert get_query_tables("ALTER TABLE target_table APPEND FROM source_table") == [
        "target_table",
        "source_table",
    ]
...
def test_table_name_with_group_by():
    ...
    assert (
        get_query_tables("SELECT s.cust_id,count(s.cust_id) FROM SH.sales s")
        == expected_tables
    )
...
def test_table_names_with_dashes():
    assert get_query_tables("SELECT * FROM `schema-with-dash.tablename`") == [
